...
// Helper functions for formatApiResponse - generated by Copilot Chat
// Prompt: In src/controllers/helpers.js in lines 451 to 522, how can I reduce the cognitive complexity?
// Function to set cache control header
const setCacheControlHeader = (res) => {
	if (res.req.loggedIn) {
		res.set('cache-control', 'private');
	}
};

// Function to map status codes to their respective messages
...
// Helper functions for formatApiResponse not generated by Copilot Chat
// Function to update status code based on error codes
const updateErrorCode = async (message, statusCode, response, res) => {
...
const assignMissingParams = (message, response) => {
...
const writeReturnPayload = (returnPayload, payload) => {
...
helpers.formatApiResponse = async (statusCode, res, payload) => {
	...
	if (String(statusCode).startsWith('2')) {
		...
		const { code, message } = getStatusMessage(statusCode);
		...
		statusCode = await updateErrorCode(message, statusCode, response, res);
		assignMissingParams(message, response);

		const returnPayload = await helpers.generateError(statusCode, message, res);
		returnPayload.response = response;
		writeReturnPayload(returnPayload, payload);
